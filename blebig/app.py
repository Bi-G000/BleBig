...
class Window(QMainWindow):
    def __init__(self):
        ...
        self._history = []; self._history_index = -1; self.update_check_worker = None; self.update_download_worker = None; self.update_progress = None
    ...
    def _snapshot(self):
        return (self.preset.currentText(), self.width_cm.value(), self.height_cm.value(),
            self.bleed_cm.value(), self.placement.currentIndex(), self.anchor.currentIndex(),
            self.mode.currentIndex(), self.dpi.value())

    def _state_changed(self, *_):
        self._refresh_summary()
        if not self._restoring: self._push_history()
    ...
    def _restore_state(self, state):
        ...
        name,w,h,bleed,placement,area,mode,dpi = state
        self.preset.setCurrentText(name); self.width_cm.setValue(w); self.height_cm.setValue(h)
        self.bleed_cm.setValue(bleed); self.placement.setCurrentIndex(placement)
        self.anchor.setCurrentIndex(area); self.mode.setCurrentIndex(mode); self.dpi.setValue(dpi)
        self._restoring = False; self._refresh_summary(); self._update_history_buttons()
    ...
    def process(self):
        ...
        output=self.output_dir or self.paths[0].parent/"BleBig_Output";mode="ai" if self.mode.currentIndex() else "fast"
        self.worker=BatchWorker(self.paths,output,layout,mode,engine_anchor_for_area(layout.area),self.dpi.value());self.worker.progress.connect(self._progress);self.worker.preview.connect(self._show_image);self.worker.completed.connect(self._completed);self.worker.failed.connect(self._failed);self.progress.setRange(0,len(self.paths));self.worker.start()
    ...
    def _completed(self,outputs):
        self.progress.setValue(len(self.paths));self.status.setText(f"Hoàn tất {len(outputs)} ảnh");QMessageBox.information(self,"BleBig",f"Đã xử lý {len(outputs)} ảnh.\nLưu tại:\n{outputs[0].parent}");QDesktopServices.openUrl(QUrl.fromLocalFile(str(outputs[0].parent)))
    def _failed(self,detail):self.status.setText("Xử lý thất bại — xem log");QMessageBox.critical(self,"BleBig",detail[-1000:])
